  ...
  _createClass(PeachDb, [{
      ...
      if (!this.initPromise) {
        this.initPromise = Promise.resolve(this.db.info.bind(this.db)).bind(this).then(this.loadMetadata).then(function () {
          ...
          return(
            ...
            _this.forcePut(_this.createDesignDoc('nameSearch', function (doc) {
              if (doc.name) {
      ...
      if (this.autoSync) {
        var _ret = (function () {
          var startAutoSync = function startAutoSync() {
            _this2.sync()['catch'](function (e) {
              throw new PeachError('Error performing initial sync', e);
            });
          };

          if (_this2.autoSyncInterval > 0) {
            _this2.$interval(function () {
              startAutoSync();
            }, _this2.autoSyncInterval);
          }
    ...
    value: function updateMetadata() {
      ...
      return this.forcePut(this.metadata).then(function (doc) {
        _this4.metadata._rev = doc.rev;
        return _this4.metadata;
      })['catch'](function (e) {
        console.error('Update metadata failed: ', e);
        return Promise.resolve(_this4.metadata);
      });
    }
    ...
    value: function load() {
      ...
      var getCount = function getCount() {
        ...
        _this5.log('getting count', currentTime.valueOf());
        howMany = 500;
        _this5.metadata.syncDate = currentTime.valueOf();
        _this5.updateMetadata();
        return _this5.restangular.one('count').get({
          selector: _this5.selector,
          createdBefore: currentTime.valueOf()
        }).then(function (r) {
          _this5.log('got the count', r);
          return r;
        });
      };

      var loadResponseItems = function loadResponseItems(response) {
        return _this5.loadItems(response.count, howMany, currentTime).then(function (r) {
          _this5.metadata.loaded = true;
          _this5.updateMetadata();
          console.timeEnd('load' + _this5.path);
          return r;
        });
      };
      ...
      return this.reducePromiseChain(this.loggerPromise('loading1'), this.whenInitialized(), this.loggerPromise('loading2'), getCount, this.loggerPromise('loading3'), loadResponseItems, this.loggerPromise('loading4'))['catch'](function (e) {
        console.error('load failed for ' + _this5.path + ' at ' + new Date() + '.', e);
      });
    }
  }, {
    key: 'loadItems',
    value: function loadItems(itemCount, howMany, currentTime) {
      ...
      return this.$q(function (resolve, reject) {
        _this6.loadItemsLoop(0, itemCount, howMany, currentTime, resolve, reject);
      });
    }
  }, {
    key: 'loadItemsLoop',
    value: function loadItemsLoop(loadedItems, itemCount, howMany, currentTime, resolve, reject) {
      ...
      this.loadChunk(loadedItems, howMany, currentTime).then(function (currentCount) {
        if (_this7.itemLimit && _this7.itemLimit < itemCount) {
          itemCount = _this7.itemLimit;
        }

        _this7.metadata.totalItemCount = itemCount;
        _this7.log('loading ', loadedItems, 'of', itemCount);
        loadedItems += howMany;
        _this7.metadata.loadedItems = loadedItems;
        if (loadedItems >= itemCount) {
          resolve(loadedItems);
        } else {
          _this7.loadItemsLoop(loadedItems, itemCount, howMany, currentTime, resolve, reject);
        }
      })['catch'](reject);
    }
  }, {
    ...
    value: function loadChunk(start, howMany, currentTime) {
      ...
      return this.restangular.getList({
        ...
        _this8.metadata.loadingIndex += data.length;
        _this8.upsertItems(data);
    ...
    value: function upsertItems() {
      ...
      var updateRev = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];
      ...
      items.forEach(function (item) {
        return item._id = _this9.idToPouchId(item.id);
      });

      // setup the pouch _id
      if (updateRev) {
        return this.syncRevisions(items).then(function (items) {
          return _this9.upsertItems(items, false);
        });
      }
      ...
      return this.db.bulkDocs(items)['catch'](function (e) {
    ...
    value: function remove(item) {
      ...
      return this.forcePut(item).then(function (res) {
        if (!localOnly) {
          _this10.deletedItems.push(item);
        }
    ...
    value: function testConnection() {
      ...
      if (new Date().getTime() - PeachNetworkStatus.lastChecked > 30000) {
        if (!navigator.onLine) {
          ...
          (function () {
            var resolved = false;
            _this11.restangular.one('').withHttpConfig({ timeout: 15000 }).head({ limit: 1 }).then(function (res) {
              resolved = true;
              deferred.resolve();
              connectionTestPassed(true);
            })['catch'](function (err) {
    ...
    value: function sync() {
      ...
      var doSync = function doSync() {
        return _this12.fetchServerData();
      };

      var ensureLoaded = function ensureLoaded() {
        if (!_this12.syncingPromise) {
          if (!_this12.isLoaded()) {
            _this12.syncingPromise = _this12.load().then(_this12.loggerPromise('load completed')).then(doSync);
          } else {
            _this12.syncingPromise = doSync();
          }

          _this12.syncingPromise.bind(_this12);
        }

        return _this12.syncingPromise;
      };

      return this.testConnection().then(function () {
        _this12.syncInProgress = true;
        return _this12.whenInitialized().then(ensureLoaded)['catch'](function (e) {
          throw new SyncError('Failed to sync', e);
        })['finally'](function () {
          _this12.syncInProgress = false;
          _this12.syncingPromise = null;
        });
      })['catch'](function () {
        console.log('Waiting for stable connection to attempt sync again');
        return _this12.$q.when();
      });
    }
  }, {
    key: 'syncDirtyItems',
      ...
      var updateDirtyDocs = function updateDirtyDocs(docs) {
        var updates = docs.map(function (doc, i) {
          if (doc._id.indexOf('new') !== 0) {
            // existing item
            return _this13.restangular.one(doc.id.toString()).customPUT(doc, null, { selector: _this13.selector }).then(function () {
              console.debug('save completed', _arguments2);
            });
          } else {
            return _this13.restangular.customPOST(doc, null, { selector: _this13.selector }).then(function (result) {
              // remove the old doc from the DB
              _this13.db.remove(doc._id, doc._rev);
              ...
              return _this13.upsertItems(_.flatten([result]));
            }, function (error) {
    ...
    value: function syncDeletedItems() {
      ...
      var deleteError = function deleteError(response) {
        ...
        console.error('Error deleting on server:', url, _this14.deletedItems, itemsToDelete, id, response);
      ...
      var deleteRequests = itemsToDelete.map(function (item) {
        ...
        return _this14.restangular.customDELETE(id).then(deleteSuccess, deleteError);
      });
      ...
      return Promise.settle(deleteRequests).then(function (responses) {
        ...
        _this14.log('checking deleted items ful/rej', fulfilled, rejected);
        _this14.deletedItems = _(rejected).map(function (promise) {
        ...
        _this14.log('updated deleted items', _this14.deletedItems);
    ...
    value: function syncNewRemoteItems() {
      ...
      var upsertNewItems = function upsertNewItems(newItems) {
        return _this15.upsertItems(newItems, true);
      };
    ...
    value: function fetchServerData() {
      ...
      var syncResultsPromise = function syncResultsPromise() {
        return Promise.settle([_this16.syncDirtyItems(), _this16.syncDeletedItems(), _this16.syncNewRemoteItems()]);
      };
      ...
      return this.reducePromiseChain(beforeSyncResults, syncResultsPromise).then(function (r) {
        ...
        _this16.metadata.syncDate = new Date().valueOf();

        if (rejections.length == 0) {
          _this16.afterSync();
        }

        return Promise.resolve(_this16.updateMetadata());
      });
    }
  }, {
    key: 'idToPouchId',
    ...
    value: function getByIds(ids) {
      ...
      var loadDocs = function loadDocs() {
        return _this17.db.allDocs({
          keys: ids.map(_this17.idToPouchId.bind(_this17)),
          include_docs: true
    ...
    value: function loggerPromise(message) {
      ...
      return Promise.method(function (r) {
        _this18.log('[Promise Logger] ', message, r);
        return r;
      });
    }
    ...
    value: function all() {
      ...
      var formatDocs = function formatDocs(rawDocs) {
        var formattedDocs = _(rawDocs.rows).map('doc').filter(function (d) {
          return d._id && (d._id.indexOf(_this19.path) === 0 || d._id.indexOf('new-' + _this19.path) === 0);
        });
      ...
      var getRawDocs = function getRawDocs() {
        return _this19.db.allDocs({ include_docs: true }).then(formatDocs);
      };

      var syncIfNeeded = function syncIfNeeded() {
        if ((_this19.isLoading() || !_this19.isLoaded()) && PeachNetworkStatus.isOnline()) {
          return _this19.sync();
        } else {
          return Promise.resolve(_this19.metadata);
        }
      };

      return this.reducePromiseChain(this.whenInitialized(), syncIfNeeded, getRawDocs)['catch'](function (e) {
        throw new PeachError('Fetching all ' + _this19.path + ' failed', e);
      });
    }
    ...
    value: function find(options) {
      ...
      return this.db.find(options).then(function (result) {
        ...
        throw new PeachError('Find failed for ' + _this20.path, e);
      });
    }
  }, {
    key: 'createIndex',
    ...
    value: function searchByName() {
      ...
      if (!splitName) {
        (function () {
          ...
          queryPromise = initCheck.then(function () {
            return _this21.db.query('nameSearch', options);
          }).then(function (results) {
        ...
        (function () {
          ...
          var getData = function getData() {
            return _this21.db.query('nameSearch', options).then(function (results) {
              _this21.allNames = results.rows;
              return results.rows;
            });
          };
          ...
          if (_this21.allNames) {
            getData = function () {
              return _this21.$q.when(_this21.allNames);
            };
          }

          var pullFilteredDocs = function pullFilteredDocs(filteredIds) {
            return _this21.db.allDocs({
              keys: filteredIds.slice(0, limit),
              include_docs: true
            });
          };

          queryPromise = initCheck.then(getData).then(_this21.loggerPromise('result of name search')).then(filterResultsAndMapIds).then(_this21.loggerPromise('after filtering search')).then(pullFilteredDocs).then(_this21.loggerPromise('after filtering')).then(function (results) {
      ...
      return this.$q.when(queryPromise['catch'](function (e) {
        throw new PeachError('Could not search for "' + query + '" on: ' + _this21.path);
      }));
    }
    ...
    value: function saveAll(items) {
      ...
      var doSync = function doSync() {
        return _this22.sync()['catch'](OfflineError, function () {})['catch'](function (e) {
          throw new PeachError('Save failed for ' + _this22.path + ' after save while' + 'attempting to sync db with server.', e);
        });
      };
      return this.$q.all(_.map(items, function (item) {
        ...
        if (!item._id) {
          item._id = 'new-' + _this22.path + ':' + Date.now() + '-' + _.random(_.now());
          localOnly = false;
        }

        return _this22.forcePut(item).then(function (item) {
          if (!localOnly) {
            _this22.dirtyItems.push(item);
          }
        });
      })).then(function () {
  ...
  _createClass(PeachModel, [{
    ...
    value: function initDb() {
      ...
      methods.forEach(function (m) {
        if (_this23[m]) return; //if the user chooses to override the method let them
        _this23[m] = _this23.peach[m].bind(_this23.peach); // have it (NOTE, you can't call super!)
      });
    }
  }, {
    key: 'beforeSync',
    ...
    value: function all() {
      ...
      var allResults = this.peach.all().then(function (items) {
        _this24.allCalled = true;
        _this24.allResults.splice(0, _this24.allResults.length);
        items.forEach(function (item) {
          _this24.allResults.push(item);
        });
        return _this24.allResults;
      });
